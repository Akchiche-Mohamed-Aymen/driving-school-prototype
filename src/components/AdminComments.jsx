...
const Comment = ({ comment  }) => {
  ...
  return (
    <>
      <div className="flex items-start space-x-3 p-2 comment  bg-gray-200 rounded-md  md:w-2xl w-full">
        {/* Left: Avatar */}
        <img
          src={comment.image}
      ...
      <p
        className="text-gray-400 comment hover:text-gray-500 pt-1 cursor-pointer md:w-2xl w-full"
  ...
  return (
    <div className="space-y-4  px-3">
      {replies.map((reply, index) => (
        <div
          key={index}
          className="flex items-start space-x-4 bg-gray-100 reply md:w-2xl    p-2   rounded-md shadow-sm"
        >
          {/* 👈 Image on the left */}
          <img
